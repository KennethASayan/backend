...
// Assign the imported data to a reactive variable
const documents = ref(internalDocumentsForApproval)

// State for the timeline tracker dialog
const isTimelineOpen = ref(false)
const selectedDocument = ref(null)

// Function to open the timeline tracker with the selected document ID
const openTimeline = (documentId) => {
  selectedDocument.value = documentId
  isTimelineOpen.value = true
}

// Function to close the timeline tracker
const closeTimeline = () => {
  ...
  <div class="bg-white rounded-lg shadow p-3 sm:p-4 md:p-6 font-sans">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6">
      <h2 class="text-lg sm:text-xl font-medium text-gray-900 mb-2 sm:mb-0">
        Internal Documents (For Approval)
      </h2>
    </div>
    ...
    <div class="block md:hidden space-y-4">
      ...
      >
        ...
        <div class="flex items-start justify-between mb-3">
          ...
          <span
            class="px-2 py-1 bg-blue-100 text-blue-700 text-xs font-semibold rounded-full whitespace-nowrap"
          >
            For Approval
          </span>
        </div>
      ...
      <div v-if="paginatedDocuments.length === 0" class="text-center py-12">
        ...
        <div class="text-xs text-gray-400 mt-1">Documents awaiting approval will appear here</div>
      </div>
    </div>
    ...
    <DocumentTimelineTracker
      ...
      document-type="internal-for-approval"
      @close="closeTimeline"
    />
  </div>
</template>
